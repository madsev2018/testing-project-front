...
class SppagebuilderAddonFeature extends SppagebuilderAddons {
	...
	public function render() {
		...
		$feature_title = '';
		if($title) {
		    $titlecolor = (isset($this->addon->settings->title_text_color) && $this->addon->settings->title_text_color) ? $this->addon->settings->title_text_color : '';
			...
			$feature_title .= '<'.$heading_selector.' '.$title_fontsizeclass.' class="sppb-addon-title sppb-feature-box-title'. $heading_class .'">';
			if( ($title_url && $url_appear == 'title') || ($title_url && $url_appear == 'both' ) ) $feature_title .= '<a '.$titlecolorclass.' href="'. $title_url .'">';
			$feature_title .= $title;
