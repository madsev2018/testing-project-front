      ...
      foreach ($items as $key => $item) {
        ...
        if(!$hide_thumbnail) {
          ...
          if ($resource == 'k2') {
            ...
            $output .= '<a class="iframe fancybox.iframe" href="'. $item->link .'&tmpl=component" itemprop="url"><img class="sppb-img-responsive" src="'. JURI::base(true) . '/'. $image .'" alt="'. $item->title .'" itemprop="thumbnailUrl"></a>';
            $output .= '</div>'; 
            }
        }
        ...
        $output .= '<div class="name-team"><a class="iframe fancybox.iframe" href="'. $item->link .'&tmpl=component" itemprop="url">' . $item->title . '</a></div>';
        ...
        if($show_readmore || $show_date) {
          ...
          if($show_readmore) {
            $output .= '<a class="iframe fancybox.iframe sppb-readmore sppb-btn sppb-btn-link" href="'. $item->link .'&tmpl=component" itemprop="url">'. $readmore_text .' <i class="fa fa-arrow-circle-o-right"></i></a>';
          }
          $output .= '</div>';
       }
