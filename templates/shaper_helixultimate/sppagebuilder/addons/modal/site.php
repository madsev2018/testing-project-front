...
class SppagebuilderAddonModal extends SppagebuilderAddons{
	...
	public function render() {
		...
		$play_icon = (isset($this->addon->settings->play_icon) && $this->addon->settings->play_icon) ? $this->addon->settings->play_icon : '';
		...
		if($modal_selector=='image') {
			//if ($selector_image) {
				$output .= '<a class="sppb-modal-selector sppb-magnific-popup" '. $attribs .' href="'. $url . '" id="'. $modal_unique_id .'-selector"><img src="' . $selector_image . '" alt="">';
					...
					if ( $modal_content_type == 'video' && $play_icon == 'yes') {
						$output  .= '<span class="icon linearicons-play"></span>';
					}
				$output  .= '</a>';
			//}
