...
/**
 * Helper for mod_swiper
 *
 * @package     Joomla.Site
 * @subpackage  mod_swiper
 */
abstract class modSwiperHelper
{
	public static function getList(&$params)
	{
		...
		foreach ($items as &$item) {
			...
			if ($access || in_array($item->access, $authorised))
			{
				...
				$item->linkText = JText::_('MOD_IMAGE_SWIPER_READMORE');
			}
			else {
				$item->link = JRoute::_('index.php?option=com_users&view=login');
				$item->linkText = JText::_('MOD_IMAGE_SWIPER_READMORE_REGISTER');
			}

			$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_image_swiper.content');
