...
class Plugin extends \JPlugin
{
    ...
    public function onConvertFormsConversionAfterSave($conversion, $model, $isNew)
    {
        ...
        catch (\Exception $e)
        {
            ...
            throw new \Exception(ucfirst($this->getName()) . ' - ' . $e->getMessage());
        }
    }
    ...
    {
        $this->loadLanguage('plg_convertforms_' . $this->getName(), JPATH_PLUGINS . '/convertforms/' . $this->getName());
    }
    ...
    protected function loadWrapper()
    {
    	...
    	{
            throw new \Exception('Wrapper ' . $wrapper .  ' not found in');
    	}
